...
class DB_Manager:
    ...
    def get_statuses(self):
        sql = sql="SELECT status_name from status" # Запиши сюда правильный SQL запрос
...
if __name__ == '__main__':
    ...
    # Заполнение таблиц skills и status
    manager.default_insert()
    user_id = 
    status_id = manager.get_status_id("Завершен")
    
    # Добавление проектов
    project_id= 1 # (user_id, project_name, project_url, status_id)
    project_name = ("MEME_DICT")
    project_url = ("https://github.com/denidadaev/MEME_DICT.git")
    status_id = ("Завершен")
   
    manager.insert_project(projects)
    
    # Добавление навыков к проектам
    skills = ("MEME_DICT", 1, "Python")  # (project_name, user_id, skill_name)
